...
def read_fits_mono(filepath: str, channel: str = "L") -> np.ndarray:
    """Read a FITS file and return a 2D float64 array."""
    s.ensure_installed("astropy")
    from astropy.io import fits as afits

    BAYER_OFFSETS = {
        "RGGB": {"R": (0, 0), "G": (0, 1), "B": (1, 1)},
        "BGGR": {"R": (1, 1), "G": (1, 0), "B": (0, 0)},
        "GRBG": {"R": (0, 1), "G": (0, 0), "B": (1, 0)},
        "GBRG": {"R": (1, 0), "G": (0, 0), "B": (0, 1)},
    }
    ...
    if data.ndim == 3:
        if channel == "R":
            data = data[0] if data.shape[0] == 3 else data[:, :, 0]
        elif channel == "G":
            data = data[1] if data.shape[0] == 3 else data[:, :, 1]
        elif channel == "B":
            data = data[2] if data.shape[0] == 3 else data[:, :, 2]
        else:
            data = data.mean(axis=0) if data.shape[0] == 3 else data.mean(axis=2)

    elif data.ndim == 2 and channel != "L":
        bayer_pat = header.get("BAYERPAT", "").strip().upper() if header else ""
        offsets   = BAYER_OFFSETS.get(bayer_pat)
        if offsets is None:
            siril.log(f"Unknown or missing Bayer pattern '{bayer_pat}', defaulting to RGGB.")
            offsets = BAYER_OFFSETS["RGGB"]
        row_off, col_off = offsets[channel]
        data = data[row_off::2, col_off::2]
...
def run_analysis(bias_path, light_path, bit_depth=16,
                 duration=60.0, gain=1.0, swamp=10.0, channel = "L", narrowband=False):
    # (logging handled by caller)

    bias_data  = read_fits_mono(bias_path, channel)
    light_data = read_fits_mono(light_path, channel)
    ...
    rel_rn_sd  = float(np.std(bias_norm, ddof=1))
    rel_rn_mad = 1.4826 * float(np.median(np.abs(bias_norm - np.median(bias_norm))))
    rel_rn     = rel_rn_mad if rel_rn_sd > 2 * rel_rn_mad else rel_rn_sd
    bias_m = float(np.median(bias_norm))

    med = float(np.median(light_norm))
    mad = 1.4826 * float(np.median(np.abs(light_norm - med)))
    light_bg = light_norm[
        (light_norm > med - 3 * mad) &
        (light_norm < med + 6 * mad)
    ]
    ...
    # v1.2.0 update fit_exgaussian call — it already returns (mu, sigma, tau)
    mu_eg, sigma_eg, tau = fit_exgaussian(light_bg)
    total_noise = float(np.sqrt(sigma_eg ** 2 + tau ** 2))
    
    #v1.2.0 Update Sky Noise based on narrowband
    if narrowband:
      light_m = float(mu_eg + tau)
    else:
      light_m = float(mu_eg)
      
    # v1.2.0 ── Sky noise: narrowband uses total ex-Gaussian noise ──────────────────
    if narrowband:
        rel_sn_total = float(np.sqrt(tau ** 2 + sigma_eg ** 2))
    else:
        rel_sn_total = float(sigma_eg)

    #rel_sn       = float(np.sqrt(max(sigma_eg ** 2 - rel_rn ** 2, 0.0)))
    rel_sn       = float(np.sqrt(max(rel_sn_total ** 2 - rel_rn ** 2, 0.0)))
    ...
    #1.2.0
    EPS      = 1e-10
    min_sub  = desired_e / (true_sn ** 2 / duration)  if abs(true_sn ** 2 / duration)  > EPS else float("inf")
    min_sub2 = desired_e / (true_sn2 ** 2 / duration) if abs(true_sn2 ** 2 / duration) > EPS else float("inf")
    #end 1.2.0
    ...
    # v1.2.0 ── Segment for histogram plot ───────────────────────────────────────────
    if narrowband:
        seg_x1 = bias_m
        seg_x2 = light_m
    else:
...
class SubLengthApp:
    ...
    def _build_ui(self):
        ...
        self.channel_var = tk.StringVar(value="L")
        ttk.Label(pf2, text="CFA Channel:", width=26, anchor="w").grid(
            row=4, column=0, sticky="w", padx=(0, 6), pady=3)
        channel_menu = ttk.Combobox(
            pf2,
            textvariable=self.channel_var,
            values=["L - Luminance (mono/average)", "R - Red", "G - Green", "B - Blue"],
            state="readonly",
            width=28,
        )
        
        self.narrowband_var = tk.StringVar(value="No")
        ttk.Label(pf2, text="Narrowband Correction:", width=26, anchor="w").grid(
            row=5, column=0, sticky="w", padx=(0, 6), pady=3)
        nb_menu = ttk.Combobox(
            pf2,
            textvariable=self.narrowband_var,
            values=["No", "Yes"],
            state="readonly",
            width=28,
        )
        
        nb_menu.grid(row=5, column=1, sticky="w")        
        
        channel_menu.grid(row=4, column=1, sticky="w")
    ...
    def _on_analyse(self):
        ...
        try:
            ...
            self.result = run_analysis(
                ...
                channel   = self.channel_var.get()[0],  # just take the first char: L, R, G or B
                narrowband = (self.narrowband_var.get() == "Yes"),   # v1.2.0
            )
            a = self.result
            self.siril.log(
